...
var InferencePoolNoMatchingPodsRouteStatus = suite.ConformanceTest{
	...
	Test: func(t *testing.T, s *suite.ConformanceTestSuite) {
		...
		expectedAcceptedRouteCond := metav1.Condition{
			...
			Status: metav1.ConditionTrue,
			// Reason is not strictly checked here by default in the helper, but can be if provided
		}

		// For GKE, "Reconciled" was the condition type seen in logs for the route's status relative to the Gateway.
		// Other controllers might use RouteConditionResolvedRefs or RouteConditionProgrammed for similar failure scenarios.
		// We are sticking to "Reconciled" as observed.
		expectedFailureRouteCond := metav1.Condition{
			Type:   string(gatewayv1.RouteConditionType("Reconciled")), // As observed in GKE logs
		...
		expectedFailureMessageSubstrings := []string{
			"missing neg status in annotation of extension service",
